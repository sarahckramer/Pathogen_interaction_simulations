...
The full analysis is conducted by running 01_run_analyses.R, which generates synthetic data and performs all five statistical methods in turn. I ran the correlation coefficients (because of their quick run time) and transfer entropy (because of the multiple dependencies) analyses locally, but ran the other methods on a high performance computing cluster. To run remotely, the code should be run once each for jobid 1-18, so that all combinations of interaction strength and duration are covered; run_local should be set as FALSE. For local running, there is code at the top of 01_run_analyses.R that can be uncommented and pasted into the console.
