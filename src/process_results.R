...
# Plot:
p.gam.1 <- ggplot(res_gam %>% mutate(sig = if_else(int_est == 'none', 'no', 'yes'))) +
  geom_violin(aes(x = as.character(theta_lambda), y = cor_median, group = theta_lambda)) +
  geom_jitter(aes(x = as.character(theta_lambda), y = cor_median, col = sig)) +
  facet_wrap(~ delta, nrow = 1) +
  theme_classic() +
  labs(title = 'GAMs')

p.gam.2 <- ggplot(data = acc_gam, aes(x = strength, y = duration, fill = perc_correct)) +
  geom_tile() +
  theme_classic() +
...
p.te.1.3 <- res_te_LIST_confound[[1]] %>%
  mutate(sig = if_else(int_est_confound2 == 'interaction', 'yes', 'no')) %>%
  ggplot(aes(x = as.character(theta_lambda), y = te_confound2, group = theta_lambda)) +
  geom_violin() +
  geom_jitter(aes(col = sig)) +
  theme_classic() +
  facet_wrap(~ 1 / delta, nrow = 1) +
  labs(title = paste0('Transfer Entropy (', names(res_te_LIST_confound)[1], ')'))
p.te.1.4 <- res_te_LIST_confound[[2]] %>%
  mutate(sig = if_else(int_est_confound2 == 'interaction', 'yes', 'no')) %>%
  ggplot(aes(x = as.character(theta_lambda), y = te_confound2, group = theta_lambda)) +
  geom_violin() +
  geom_jitter(aes(col = sig)) +
  theme_classic() +
  facet_wrap(~ 1 / delta, nrow = 1) +
  labs(title = paste0('Transfer Entropy (', names(res_te_LIST_confound)[2], ')'))
grid.arrange(p.te.1.1, p.te.1.2, p.te.1.3, p.te.1.4, ncol = 1)
...
p.te.2.3 <- ggplot(data = acc_te_LIST[[3]], aes(x = strength, y = duration, fill = perc_correct)) +
  geom_tile() +
  theme_classic() +
  scale_fill_viridis(option = 'G', limits = c(0, 1)) +
  labs(title = paste0('Transfer Entropy (w/ Confounding) (', names(acc_te_LIST)[3], ')'))
p.te.2.4 <- ggplot(data = acc_te_LIST[[4]], aes(x = strength, y = duration, fill = perc_correct)) +
  geom_tile() +
  theme_classic() +
  scale_fill_viridis(option = 'G', limits = c(0, 1)) +
  labs(title = paste0('Transfer Entropy (w/ Confounding) (', names(acc_te_LIST)[4], ')'))


rm(p.te.1.1, p.te.1.2, p.te.1.3, p.te.1.4, res_te_LIST, res_te_LIST_confound, best_v1xv2, best_v2xv1, best_v1xv2_confound, best_v2xv1_confound)
...
rm(p.ccm.1.1, p.ccm.1.2, p.ccm.1.3, p.ccm.1.4, p.ccm.1.5, p.ccm.1.6, res_ccm_LIST, res_ccm)
...
# Heatmaps:
p.comb.1 <- arrangeGrob(p.corr.3, p.gam.2, p.granger.3.1, p.granger.3.2, p.granger.3.3, p.granger.3.4, p.te.2.1, p.te.2.2, p.te.2.3, p.te.2.4, p.ccm.3.1, p.ccm.3.4, p.ccm.3.2, p.ccm.3.5,
...
rm(p.corr.3,p.gam.2, p.granger.3.1, p.granger.3.2, p.granger.3.3, p.granger.3.4, p.te.2.1, p.te.2.2,
   p.te.2.3, p.te.2.4, p.ccm.3.1, p.ccm.3.4, p.ccm.3.2, p.ccm.3.5, p.ccm.3.3, p.ccm.3.6)
...
res_acc <- acc_corr %>% mutate(method = 'Corr. Coef.', direction = 'v1 -> v2') %>%
  ...
  bind_rows(acc_te_LIST[[3]] %>% mutate(method = 'TE (w/ Seas)', direction = 'v1 -> v2')) %>%
  bind_rows(acc_te_LIST[[4]] %>% mutate(method = 'TE (w/ Seas)', direction = 'v2 -> v1')) %>%
  bind_rows(acc_ccm_LIST[[1]] %>% mutate(method = 'CCM (Method 1)', direction = 'v1 -> v2')) %>%
  bind_rows(acc_ccm_LIST[[4]] %>% mutate(method = 'CCM (Method 1)', direction = 'v2 -> v1')) %>%
  bind_rows(acc_ccm_LIST[[2]] %>% mutate(method = 'CCM (Method 2)', direction = 'v1 -> v2')) %>%
  bind_rows(acc_ccm_LIST[[5]] %>% mutate(method = 'CCM (Method 2)', direction = 'v2 -> v1')) %>%
  mutate(method = factor(method, levels = c('Corr. Coef.', 'GAMs', 'GC', 'GC (w/ Seas.)', 'TE', 'TE (w/ Seas)', 'CCM (Method 1)', 'CCM (Method 2)'))) %>%
...
p.comb.2 <- ggplot(res_acc, aes(x = x_use, y = perc_correct, group = method, shape = method, col = method)) +
  ...
  scale_shape_manual(values = c(18, 17, 15, 15, 3, 3, 8, 8)) +
  scale_color_manual(values = c('#ff7f00', '#e31a1c', '#1f78b4', '#a6cee3', '#6a3d9a', '#cab2d6', '#33a02c', '#b2df8a')) +
  labs(x = 'Interaction Strength', y = '% Correct', shape = 'Method', col = 'Method')
print(p.comb.2)
rm(acc_corr, acc_gam, acc_granger_LIST, acc_te_LIST, acc_ccm_LIST)
...
res_assoc <- assoc_corr %>% mutate(method = 'Corr. Coef.', direction = 'v1 -> v2') %>%
  ...
  bind_rows(assoc_te_LIST[[3]] %>% mutate(method = 'TE (w/ Seas)', direction = 'v1 -> v2')) %>%
  bind_rows(assoc_te_LIST[[4]] %>% mutate(method = 'TE (w/ Seas)', direction = 'v2 -> v1')) %>%
  bind_rows(assoc_ccm_LIST_max[[1]] %>% mutate(method = 'CCM (Method 1)', direction = 'v1 -> v2')) %>%
  bind_rows(assoc_ccm_LIST_max[[4]] %>% mutate(method = 'CCM (Method 1)', direction = 'v2 -> v1')) %>%
  bind_rows(assoc_ccm_LIST_max[[2]] %>% mutate(method = 'CCM (Method 2)', direction = 'v1 -> v2')) %>%
  bind_rows(assoc_ccm_LIST_max[[5]] %>% mutate(method = 'CCM (Method 2)', direction = 'v2 -> v1')) %>%
  mutate(method = factor(method, levels = c('Corr. Coef.', 'GAMs', 'GC', 'GC (w/ Seas.)', 'TE', 'TE (w/ Seas)', 'CCM (Method 1)', 'CCM (Method 2)'))) %>%
...
p.comb.4 <- ggplot(df_acc %>% filter(!is.na(method)), aes(x = direction, y = weighted_acc, shape = method, col = method)) +
  geom_point(size = 3) +
