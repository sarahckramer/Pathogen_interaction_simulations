...
df_acc <- acc_corr %>% mutate(method = 'Corr. Coef.') %>%
  ...
  bind_rows(acc_ccm1_v1v2 %>% mutate(method = 'CCM (Method 2) (A %->% B)')) %>%
  bind_rows(acc_ccm1_v2v1 %>% mutate(method = 'CCM (Method 2) (B %->% A)')) %>%
  bind_rows(acc_ccm2_v1v2 %>% mutate(method = 'CCM (Method 1) (A %->% B)')) %>%
  bind_rows(acc_ccm2_v2v1 %>% mutate(method = 'CCM (Method 1) (B %->% A)'))
...
p_acc_forcing_c <- ggplot(data = df_by_amp %>% arrange(forcing) %>% mutate(line_col = forcing + 0.05) %>% filter(method == 'GC (A %->% B)'),
  ...
  scale_y_continuous(limits = c(0, 1), n.breaks = 6) +
  scale_color_viridis(limits = c(0, 0.5)) +
  labs(x = NULL, y = NULL, title = 'GC (A \u279E B)')
p_acc_forcing_d <- ggplot(data = df_by_amp %>% arrange(forcing) %>% mutate(line_col = forcing + 0.05) %>% filter(method == 'GC (B %->% A)'),
  ...
  scale_y_continuous(limits = c(0, 1), n.breaks = 6) +
  scale_color_viridis(limits = c(0, 0.5)) +
  labs(x = NULL, y = NULL, title = 'GC (B \u279E A)')
p_acc_forcing_e <- ggplot(data = df_by_amp %>% arrange(forcing) %>% mutate(line_col = forcing + 0.05) %>% filter(method == 'TE (A %->% B)'),
  ...
  scale_y_continuous(limits = c(0, 1), n.breaks = 6) +
  scale_color_viridis(limits = c(0, 0.5)) +
  labs(x = NULL, y = NULL, title = 'TE (A \u279E B)')
p_acc_forcing_f <- ggplot(data = df_by_amp %>% arrange(forcing) %>% mutate(line_col = forcing + 0.05) %>% filter(method == 'TE (B %->% A)'),
  ...
  scale_y_continuous(limits = c(0, 1), n.breaks = 6) +
  scale_color_viridis(limits = c(0, 0.5)) +
  labs(x = NULL, y = NULL, title = 'TE (B \u279E A)')
p_acc_forcing_g <- ggplot(data = df_by_amp %>% arrange(forcing) %>% mutate(line_col = forcing + 0.05) %>% filter(method == 'CCM (Method 1) (A %->% B)'),
  ...
  scale_y_continuous(limits = c(0, 1), n.breaks = 6) +
  scale_color_viridis(limits = c(0, 0.5)) +
  labs(x = NULL, y = NULL, title = 'CCM (Method 1) (A \u279E B)')
p_acc_forcing_h <- ggplot(data = df_by_amp %>% arrange(forcing) %>% mutate(line_col = forcing + 0.05) %>% filter(method == 'CCM (Method 1) (B %->% A)'),
  ...
  scale_y_continuous(limits = c(0, 1), n.breaks = 6) +
  scale_color_viridis(limits = c(0, 0.5)) +
  labs(x = NULL, y = NULL, title = 'CCM (Method 1) (B \u279E A)')
p_acc_forcing_i <- ggplot(data = df_by_amp %>% arrange(forcing) %>% mutate(line_col = forcing + 0.05) %>% filter(method == 'CCM (Method 2) (A %->% B)'),
  ...
  scale_y_continuous(limits = c(0, 1), n.breaks = 6) +
  scale_color_viridis(limits = c(0, 0.5)) +
  labs(x = NULL, y = NULL, title = 'CCM (Method 2) (A \u279E B)')
p_acc_forcing_j <- ggplot(data = df_by_amp %>% arrange(forcing) %>% mutate(line_col = forcing + 0.05) %>% filter(method == 'CCM (Method 2) (B %->% A)'),
  ...
  scale_y_continuous(limits = c(0, 1), n.breaks = 6) +
  scale_color_viridis(limits = c(0, 0.5)) +
  labs(x = NULL, y = NULL, title = 'CCM (Method 2) (B \u279E A)')
...
p_acc_forcing <- arrangeGrob(p_acc_forcing_legend,
                             arrangeGrob(p_acc_forcing_a, p_acc_forcing_b, p_acc_forcing_c, p_acc_forcing_d,
...
plot(p_acc_forcing)
# ggsave(filename = 'results/plots/figures/FigureS7.svg', p_acc_forcing, width = 6.1, height = 15.2)
...
df_acc <- df_acc %>%
  mutate(which_sens = case_when(which_sens == '20y' ~ '20 years',
                                ...
                                .default = which_sens)) %>%
  mutate(which_sens = factor(which_sens, levels = c('Main', 'High obs noise', 'Low process noise', 'No obs noise', '20 years'))) %>%
...
p_acc_a <- ggplot(data = df_acc %>% filter(which_sens != 'Main', method == 'Corr. Coef.')) +
  ...
  theme(axis.title = element_text(size = 14),
        ...
        panel.grid.minor = element_blank(),
        legend.position = 'none') +
  scale_x_continuous(limits = c(0.3, 1), n.breaks = 6) +
  scale_y_continuous(limits = c(0, 0.9), n.breaks = 6) +
  scale_color_manual(values = c('#7570b3', '#e6ab02', '#1b9e77', '#e7298a')) +
  labs(x = NULL, y = NULL, title = 'Corr. Coef.')
...
p_acc <- arrangeGrob(p_acc_legend,
                     arrangeGrob(p_acc_a, p_acc_b, p_acc_c, p_acc_d, p_acc_e, p_acc_f, p_acc_g, p_acc_h, p_acc_i, p_acc_j,
                                 ncol = 2, left = y_lab, bottom = x_lab),
