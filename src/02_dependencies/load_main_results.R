...
if (sensLagEmbedding) {
  res_filenames_T <- list.files(path = 'results/sensLE/', pattern = 'TRUE', full.names = TRUE) # run locally
  res_filenames_F <- list.files(path = 'results/sensLE', pattern = 'FALSE', full.names = TRUE) # run on cluster
}
...
if (sensLagEmbedding) {
  res_granger %>%
    group_by(direction, confounding) %>%
    summarise(mean = mean(lag_order),
              median = median(lag_order),
              min = min(lag_order),
              max = max(lag_order)) %>%
    print()
}
...
if (sensLagEmbedding) {
  res_te_confound1 <- res_te %>% group_by(direction, run, .id) %>% filter(te_confound == max(te_confound)) %>% ungroup() %>% select(-c(te, sd_null, p_value, int_est))
} else {
...
if (sensLagEmbedding) {
  ...
  res_ccm <- res_ccm %>%
    ...
    select(run:direction, rho_median, tp_use:p_surr, theta_lambda:delta) %>%
    summarise(rho_mean = mean(rho_median), rho_max = rho_median[LibSize == max(LibSize)], max_cmc = unique(max_cmc), p_conv = unique(p_conv), p_surr = unique(p_surr)) %>%
    ungroup()
  ...
  res_ccm <- res_ccm %>%
    select(run:direction, rho_max, p_surr, p_conv, theta_lambda:delta, int_true:int_est_2)
...
if (sensLagEmbedding) {
  res_ccm_LIST[[1]] <- res_ccm %>% filter(direction == 'v1 -> v2') %>% select(-int_est_2) %>% rename('int_est' = 'int_est_1')
  res_ccm_LIST[[2]] <- res_ccm %>% filter(direction == 'v1 -> v2') %>% select(-int_est_1) %>% rename('int_est' = 'int_est_2')
  res_ccm_LIST[[4]] <- res_ccm %>% filter(direction == 'v2 -> v1') %>% select(-int_est_2) %>% rename('int_est' = 'int_est_1')
  res_ccm_LIST[[5]] <- res_ccm %>% filter(direction == 'v2 -> v1') %>% select(-int_est_1) %>% rename('int_est' = 'int_est_2')
} else {
