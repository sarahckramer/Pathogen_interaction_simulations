...
# Load and process results (round 2)

# Read in results:
res_dir <- 'results/trajectory_matching/round2/'
...
# Write start ranges to file:
# write_rds(ci_start, file = 'results/trajectory_matching/round3CI_startvals.rds')
rm(ci_start)

# ---------------------------------------------------------------------------------------------------------------------

# Load and process results (round 3)

# Read in results:
res_dir <- 'results/trajectory_matching/round3/'
...
for (int_set in 1:16) {
  ...
  for (i in seq_along(ids_to_fit)) {
    ...
    res_list_TEMP[[i]] <- load_and_format_results(list.files(path = res_dir, pattern = pat_temp, full.names = TRUE), get_top_5_perc = FALSE) %>%
      mutate(int_set = int_set, .id = data_id, .before = 'Ri1')
    rm(data_id, pat_temp)
...
# Set up round 4, where necessary

# Identify parameter sets needing further runs:
res_rerun <- res %>%
  group_by(int_set, .id) %>%
  summarise(n_best = n()) %>%
  filter(n_best < 5)

# Read in results for just these datasets, to set up round 4:
res_LIST = vector('list', length = 16)
for (run in 1:16) {
  print(run)
  ...
  for (i in seq_along(ids_to_fit)) {
    ...
    # Check whether we need to read in these results:
    res_temp <- res_rerun %>%
      filter(int_set == run,
             .id == data_id)
    ...
    if (nrow(res_temp) == 1) {
      ...
      pat_temp <- paste0('_', run, '_', data_id, '_')
      print(pat_temp)
      
      res_list_TEMP[[i]] <- load_and_format_results(list.files(path = res_dir, pattern = pat_temp, full.names = TRUE), get_top_5_perc = TRUE) %>%
        mutate(int_set = run, .id = data_id, .before = 'Ri1')
  ...
  res_LIST[[run]] <- bind_rows(res_list_TEMP)
  rm(res_list_TEMP)
}
rm(run, i)
...
# Check correct results read in:
expect_true(
  res %>%
    select(int_set:.id) %>%
    distinct() %>%
    arrange(int_set, .id) %>%
    all.equal(res_rerun %>% select(-n_best) %>% ungroup())
)
rm(res_rerun)
...
# Write start ranges to file:
# write_rds(ci_start, file = 'results/trajectory_matching/round4CI_startvals.rds')
rm(ci_start)
